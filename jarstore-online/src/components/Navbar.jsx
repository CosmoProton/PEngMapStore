...
import { BookOpen, Shield, LogOut, Home, Info, Calendar, ClipboardList } from 'lucide-react'; // Aggiunte icone compiti
          ...
          {user ? (
            <>
              ...
              <div style={S.links} className="nav-links">
                ...
                {/* NUOVO: Link Compiti per tutti gli studenti loggati */}
                <Link to="/tasks" style={{...S.link, ...(pathname==='/tasks'?S.linkOn:{})}}>
                  <Calendar size={16}/> <span className="hide-mobile-text">Compiti</span>
                </Link>
                ...
                {isAdmin && (
                  <>
                    ...
                    {/* NUOVO: Link Registro Compiti solo per Admin/Docenti */}
                    <Link to="/admin/tasks" style={{...S.link, ...(pathname==='/admin/tasks'?S.linkOn:{})}}>
                      <ClipboardList size={16}/> <span className="hide-mobile-text">Task Admin</span>
                    </Link>
                  </>
                )}
              </div>
